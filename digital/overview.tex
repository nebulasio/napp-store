...
\[(r, g, m^{\prime})\]
Payment Checker在收到相应的内容之后，需要验证\(m^{\prime}\)对应的hash正是\(g\)，此处需要使用{\color{red} 零知识证明？}，如\reffig{fig:buy}中\circled{4}所示。在验证通过后，Payment Checker将相应的NAS发放给作者，并将相应的密文发送给读者，如\reffig{fig:buy}中\circled{5}所示。
